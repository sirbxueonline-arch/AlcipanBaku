    ...
    return (
        <section className="py-12 bg-[#f8fafc]" id="pricing">
            <div className="container mx-auto px-4">
                ...
                {/* Packages Grid */}
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6 max-w-7xl mx-auto">
                    {PACKAGES.map((pkg, index) => (
                        <motion.div 
                            ...
                            viewport={{ once: true }}
                            className="bg-white rounded-xl md:rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 flex flex-col"
                        >
                            {/* Card Header - Dark Blue */}
                            <div className="bg-[#0a192f] p-2 md:p-4 text-center py-4 md:py-6 relative overflow-hidden">
                                ...
                                <h3 className="text-sm md:text-xl font-bold text-white tracking-wide">
                                    {pkg.area} m² <span className="hidden md:inline">{language === 'AZ' ? 'Paket' : language === 'RU' ? 'Пакет' : 'Package'}</span>
                                </h3>
                                <div className="text-[8px] md:text-[10px] text-gray-400 mt-0.5 md:mt-1 uppercase tracking-wider">
                                    {language === 'AZ' ? 'TAM XİDMƏT' : language === 'RU' ? 'ПОЛНЫЙ СЕРВИС' : 'FULL SERVICE'}
                                </div>
                            </div>

                            {/* Card Body */}
                            <div className="p-3 md:p-6 flex flex-col items-center flex-1">
                                
                                <div className="text-center mb-3 md:mb-6">
                                     <div className="flex items-baseline justify-center gap-0.5 md:gap-1">
                                        <span className="text-xl md:text-3xl font-extrabold text-[#0a192f]">{pkg.price}</span>
                                        <span className="text-sm md:text-xl font-bold text-[#0a192f]">AZN</span>
                                     </div>
                                </div>
                                
                                {/* Features List */}
                                <ul className="w-full space-y-1.5 md:space-y-3 mb-4 md:mb-8">
                                    {[
                                        ...
                                        <li key={i} className="flex items-center gap-1.5 md:gap-3 text-[10px] md:text-sm text-gray-600">
                                            <svg className="w-3 h-3 md:w-4 md:h-4 text-[#fbbf24] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                                            </svg>
                                            <span className="leading-tight">{language === 'AZ' ? feature.az : language === 'RU' ? feature.ru : feature.en}</span>
                                        </li>
                                    ))}
                                </ul>
